...
public class ReleaseVirtualMachineDeepTest {
  ...
  public void getThisReturnsNullWhenNoFrameIsActive() {
    // Before any method invocation, getThis should be null
    // (no current frame on this thread)
    // We access it indirectly through a static method that doesn't set up "this"
    ...
    // Static methods have no "this" — the frame's getThis returns null
    vm.ENTRY_POINT_invoke(null, method);
  ...
  public void localDeclarationAndAccessRoundTripsWithDifferentTypes() {
    ...
    org.lgna.project.ast.ReturnStatement ret = new org.lgna.project.ast.ReturnStatement(
        JavaType.getInstance(Integer.class),
        new org.lgna.project.ast.LocalAccess(intLocal));
