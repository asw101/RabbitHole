...
public class VmStatementExecutorDeepTest {
  ...
  public void nestedCountLoopsFireCorrectIterationCounts() {
    ...
    // 3 outer iterations * 2 inner iterations = 6 inner body executions
    long commentCount = listener.statementEvents.stream()
        .filter(e -> e.equals("executing:Comment")).count();
  ...
  public void conditionalWithMultipleBranchesSelectsFirstTrueClause() {
    ...
    // Only 1 comment should fire — from the second (true) branch
    long commentCount = listener.statementEvents.stream()
        .filter(e -> e.equals("executing:Comment")).count();
  ...
  public void whileLoopBodyExecutionIsCounted() {
    // This mimics a while loop that runs exactly once by using a countLoop-style pattern
    // We test via a method that returns a value after the loop body modifies a local
    ...
    // We can't easily create a mutable condition without field access,
    // so test the false-condition variant fully
