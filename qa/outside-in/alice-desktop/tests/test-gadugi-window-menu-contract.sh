...
SCENARIO="$BASE_DIR/gadugi/window-menu-registration-evidence.yaml"

assert_file_exists "$SCENARIO" "Gadugi Window menu registration evidence scenario exists"

python3 - "$SCENARIO" >"$tmp_root/gadugi-window-menu-contract.out" 2>"$tmp_root/gadugi-window-menu-contract.err" <<'PY'
...
expected_name = "window-menu-registration-evidence"
...
for required in (
    "pr #401",
    "window menu model registration",
...
for tag in ("cli", "gadugi", "pr-401", "menu-action", "window-menu-registration-evidence"):
    ...
    expected_commands = [
        ...
        (
            ...
            (
                ...
                "--prepare-only",
                "--evidence-dir",
                "qa/outside-in/alice-desktop/evidence/gadugi-window-menu-registration",
            ),
        ),
...
if assertions != []:
    errors.append("scenario should not add Gadugi assertions that overclaim menu registration or full workflow")
