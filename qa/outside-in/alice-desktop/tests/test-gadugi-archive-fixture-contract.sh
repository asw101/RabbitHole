...
SCENARIO="$BASE_DIR/gadugi/archive-fixture-evidence.yaml"

assert_file_exists "$SCENARIO" "Gadugi archive fixture evidence scenario exists"

python3 - "$SCENARIO" >"$tmp_root/gadugi-archive-fixture-contract.out" 2>"$tmp_root/gadugi-archive-fixture-contract.err" <<'PY'
...
expected_name = "archive-fixture-evidence"
...
for required in (
    "pr #433",
    "legacy fixture round-trip characterization",
...
for required_non_claim in (
    "does not validate full historical archive migration",
    "full tweedle decode",
    "full player decode",
    "arbitrary user archive support",
    "ui automation",
    "visible rendering",
    "desktop save/open behavior",
):
...
for tag in ("cli", "gadugi", "pr-433", "archive-fixture", "archive-fixture-evidence"):
    ...
    expected_commands = [
        ...
        (
            ...
            (
                ...
                "--prepare-only",
                "--evidence-dir",
                "qa/outside-in/alice-desktop/evidence/gadugi-archive-fixture",
            ),
        ),
...
if top_level_scalar("assertions") != "[]":
    errors.append("scenario should not add Gadugi assertions that overclaim archive fixture correctness")
