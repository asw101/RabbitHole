...
SCENARIO="$BASE_DIR/gadugi/run-window-contract-evidence.yaml"

assert_file_exists "$SCENARIO" "Gadugi run-window contract evidence scenario exists"

python3 - "$SCENARIO" >"$tmp_root/gadugi-run-window-contract.out" 2>"$tmp_root/gadugi-run-window-contract.err" <<'PY'
...
expected_name = "run-window-contract-evidence"
...
for required_non_claim in (
    "does not validate active rendering",
    "run execution",
...
for tag in ("cli", "gadugi", "pr-434", "run-window-contract", "run-window-evidence"):
...
if assertions != []:
    errors.append("scenario should not add Gadugi assertions that overclaim run-window rendering or full workflow")
