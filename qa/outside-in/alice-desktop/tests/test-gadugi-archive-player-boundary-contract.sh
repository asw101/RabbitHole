...
SCENARIO="$BASE_DIR/gadugi/archive-player-boundary-evidence.yaml"

assert_file_exists "$SCENARIO" "Gadugi archive player boundary evidence scenario exists"

python3 - "$SCENARIO" >"$tmp_root/gadugi-archive-player-boundary-contract.out" 2>"$tmp_root/gadugi-archive-player-boundary-contract.err" <<'PY'
...
expected_name = "archive-player-boundary-evidence"
...
for tag in ("cli", "gadugi", "pr-438", "archive-player-boundary", "archive-boundary-evidence-contract"):
    ...
    expected_commands = [
        ...
        (
            ...
            (
                ...
                "--prepare-only",
                "--evidence-dir",
                "qa/outside-in/alice-desktop/evidence/gadugi-archive-player-boundary",
            ),
        ),
...
if top_level_scalar("assertions") != "[]":
    errors.append("scenario should not add Gadugi assertions that overclaim archive boundary rendering or full workflow")
