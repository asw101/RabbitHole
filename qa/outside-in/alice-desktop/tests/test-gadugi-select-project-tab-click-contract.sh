...
SCENARIO="$BASE_DIR/gadugi/select-project-tab-click-evidence.yaml"

assert_file_exists "$SCENARIO" "Gadugi select-project tab-click evidence scenario exists"

python3 - "$SCENARIO" >"$tmp_root/gadugi-select-project-tab-click-contract.out" 2>"$tmp_root/gadugi-select-project-tab-click-contract.err" <<'PY'
...
expected_name = "select-project-tab-click-evidence"
...
for required in (
    "pr #437",
    "select-project tab-click evidence workflow",
    "at-spi selection/open evidence contract",
):
...
for tag in ("cli", "gadugi", "pr-437", "select-project-tab-click", "tab-click-evidence-contract"):
    ...
    expected_commands = [
        ...
        (
            ...
            (
                ...
                "--prepare-only",
                "--evidence-dir",
                "qa/outside-in/alice-desktop/evidence/gadugi-select-project-tab-click",
            ),
        ),
...
if top_level_scalar("assertions") != "[]":
    errors.append("scenario should not add Gadugi assertions that overclaim tab-click rendering or full workflow")
