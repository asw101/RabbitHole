#!/usr/bin/env bash
# qa/outside-in/alice-desktop/tests/test-gadugi-compatibility-contract.sh
set -eu
...
SCENARIO="$BASE_DIR/gadugi/run-render-affordance-evidence.yaml"

assert_file_exists "$SCENARIO" "Gadugi Run render-affordance evidence scenario exists"
...
expected_name = "run-render-affordance-evidence"
...
for tag in ("cli", "gadugi", "pr-154", "desktop-run-debug", "render-affordance-evidence"):
    ...
    expected_commands = [
        ...
        (
            ...
            (
                ...
                "--prepare-only",
                "--evidence-dir",
                "qa/outside-in/alice-desktop/evidence/gadugi-run-render-affordance",
            ),
        ),
...
if assertions != []:
    errors.append("scenario should not add Gadugi assertions that overclaim rendering correctness")
