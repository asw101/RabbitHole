...
| Select Project tab-click smoke | Target contract: `tab-click-observation.json` with exact `Africa Full` `targetStarter` metadata, `evidenceStatus=opened`, safe `startersTabSafety` (`activatedBeforeTargetSearch=true`, `targetSearchScope=active-starters-tab`), `targetSelectionObserved=true`, `openAttempted=true`, and `projectOpenObserved=true`, or existing `blocker`/`blockerDetail` fields plus structured target-specific `nextBlocker` details. See [Select Project Africa Full AT-SPI evidence reference](./select-project-africa-full-atspi-evidence.md). |
...
| Post-project open window-state smoke | `post-project-open-observation.json` characterizing main-window AT-SPI state. The gate requires prior `tab-click-observation.json` Africa Full evidence with `evidenceStatus=opened`, exact target metadata, safe `startersTabSafety`, `targetSelectionObserved=true`, `openAttempted=true`, and `projectOpenObserved=true`; generic main-window presence is not Africa Full proof. |
