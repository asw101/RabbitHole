...
| `Not3dHandleCriterion` (class) | N/A (anonymous) | package-private | No external consumers; both external usage sites (`ResizeDragManipulator`, `ScaleDragManipulator`) reference the `NOT_3D_HANDLE_CRITERION` constant |
...
| `invertParentScale()` (method) | `private` on `ManipulationHandle3D` | `static` on `HandleGeometryHelper` | Reads and writes the target's `localTransformation` via the passed `Transformable` and `Composite` arguments |
